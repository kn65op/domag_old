...
TEST(getIntFromNColumn, gettingInts)
{
  ...
  EXPECT_TRUE(sql->getNextRecord());
  EXPECT_THROW(sql->getIntFromNColumn(0), NoDataException);
  EXPECT_THROW(sql->getIntFromNColumn(4), WrongDataException);
  EXPECT_THROW(sql->getIntFromNColumn(5), NoDataException);
  EXPECT_EQ(2, sql->getIntFromNColumn(1));
  EXPECT_THROW(sql->getIntFromNColumn(2), WrongDataException);
...
TEST(getStringFromNColumn, gettingStrings)
{
  ...
  EXPECT_STREQ("123", sql->getStringFromNColumn(2));
  EXPECT_STREQ("qwe", sql->getStringFromNColumn(3));
  ASSERT_TRUE(sql->getNextRecord());
  EXPECT_STREQ("456", sql->getStringFromNColumn(2));
  EXPECT_STREQ("asd", sql->getStringFromNColumn(3));
  EXPECT_FALSE(sql->getNextRecord());
  EXPECT_NO_THROW(sql->executeSelectQuery("SELECT a, b from first;"));
  EXPECT_THROW(sql->getStringFromNColumn(1), NoDataException);
  EXPECT_TRUE(sql->getNextRecord());
  ...
  EXPECT_STREQ("123", sql->getStringFromNColumn(1));
  EXPECT_STREQ("qwe", sql->getStringFromNColumn(2));
  EXPECT_TRUE(sql->getNextRecord());
  EXPECT_STREQ("456", sql->getStringFromNColumn(1));
  EXPECT_STREQ("asd", sql->getStringFromNColumn(2));
  EXPECT_FALSE(sql->getNextRecord());
  EXPECT_NO_THROW(sql->executeSelectQuery("SELECT b, 'qwe', 4, 5.5 from first;"));
  EXPECT_THROW(sql->getStringFromNColumn(1), NoDataException);
  EXPECT_TRUE(sql->getNextRecord());
  EXPECT_THROW(sql->getStringFromNColumn(0), NoDataException);
  EXPECT_THROW(sql->getStringFromNColumn(4), WrongDataException);
  EXPECT_THROW(sql->getStringFromNColumn(5), NoDataException);
  EXPECT_STREQ("qwe", sql->getStringFromNColumn(1));
  ...
  EXPECT_THROW(sql->getStringFromNColumn(4), WrongDataException);
  EXPECT_THROW(sql->getStringFromNColumn(5), NoDataException);
  EXPECT_STREQ("asd", sql->getStringFromNColumn(1));
  EXPECT_THROW(sql->getStringFromNColumn(3), WrongDataException);
  EXPECT_STREQ("wqe", sql->getStringFromNColumn(2));
  EXPECT_FALSE(sql->getNextRecord());
  EXPECT_NO_THROW(sql->executeQuery("DROP TABLE first;"));
  delete sql;
}
...
TEST(getDoubleFromNColumn, gettingInts)
{
  ...
  EXPECT_EQ(1.1, sql->getDoubleFromNColumn(1));
  EXPECT_EQ(0.2, sql->getDoubleFromNColumn(2));
  EXPECT_EQ(3.0, sql->getDoubleFromNColumn(3));
  ASSERT_TRUE(sql->getNextRecord());
  EXPECT_EQ(2.5, sql->getDoubleFromNColumn(1));
  EXPECT_EQ(0.05, sql->getDoubleFromNColumn(2));
  EXPECT_EQ(6.0, sql->getDoubleFromNColumn(3));
  EXPECT_FALSE(sql->getNextRecord());
  EXPECT_NO_THROW(sql->executeSelectQuery("SELECT a, b from first;"));
  EXPECT_THROW(sql->getDoubleFromNColumn(1), NoDataException);
  EXPECT_TRUE(sql->getNextRecord());
  ...
  EXPECT_FALSE(sql->getNextRecord());
  EXPECT_NO_THROW(sql->executeSelectQuery("SELECT b, 'qwe', 5.5, 4 from first;"));
  EXPECT_THROW(sql->getDoubleFromNColumn(1), NoDataException);
  EXPECT_TRUE(sql->getNextRecord());
  EXPECT_THROW(sql->getDoubleFromNColumn(0), NoDataException);
  EXPECT_THROW(sql->getDoubleFromNColumn(4), WrongDataException);
  EXPECT_THROW(sql->getDoubleFromNColumn(5), NoDataException);
  EXPECT_EQ(0.2, sql->getDoubleFromNColumn(1));
  EXPECT_THROW(sql->getDoubleFromNColumn(2), WrongDataException);
  EXPECT_EQ(5.5, sql->getDoubleFromNColumn(3));
  EXPECT_TRUE(sql->getNextRecord());
  EXPECT_THROW(sql->getDoubleFromNColumn(0), NoDataException);
  EXPECT_THROW(sql->getDoubleFromNColumn(4), WrongDataException);
  EXPECT_THROW(sql->getDoubleFromNColumn(5), NoDataException);
